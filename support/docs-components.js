    ...
    render() {
        ...
        this.innerHTML = navItems.map(item => {
            ...
            if (activeTab) {
                ...
                if (item.id === 'guides' && (currentPath.endsWith('/support/') || currentPath.includes('/support/'))) {
                    isActive = true;
                } else if (item.id === 'app' && !currentPath.includes('/support/')) {
                    isActive = true;
    ...
    highlightActiveLink() {
        ...
        if (h1 && !document.querySelector('.doc-page-category') && !hideCategory) {
            ...
            fetch(`${basePath}search-index.json`)
                ...
                .then(index => {
                    const currentPath = window.location.pathname;
                    const match = index.find(item => currentPath.endsWith(item.path) || item.path.includes(currentPath.split('/support/')[1]));
